...
public final class SqlResultHelper {
  ...
  /**
   * Executes a multi-statement SQL string (semicolon-separated) as individual commands.
   */
  public static void exec(GraphTraversalSource g, String sql, Object... keyValues) {
    for (String stmt : sql.split(";")) {
      String trimmed = stmt.trim();
      if (!trimmed.isEmpty()) {
        ytdb(g).sqlCommand(trimmed, keyValues).iterate();
      }
    }
  }
